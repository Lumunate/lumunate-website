...
export default function WorkflowCard({ activeSection }: WorkflowCardProps) {
  ...
  useEffect(() => {
    ...
    if (textElementsRef.current) {
      const children =
        textElementsRef.current.querySelectorAll(".animate-text");
      gsap.fromTo(
        children,
    ...
    <WorkflowCardRoot ref={cardRef}>
      <CardLeft>
        <Box ref={textElementsRef}>
          <TopText className="animate-text" variant="h6" component="span">
            {activeSection.tag}
          </TopText>

          <Title className="animate-text" variant="h4" component="h2">
            {activeSection.title}
          </Title>

          <Description className="animate-text" component="p">
        ...
        <Link
          ...
          aria-label={`Discover more about our ${activeSection.title} solutions`}
        >
          <DiscoverButton
            aria-label={`Discover more about ${activeSection.title || "our process"}`}
            sx={{ mt: { xs: "0px", md: "0px" }, width: "fit-content" }}
          >
